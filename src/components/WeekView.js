...
const WeekView = ({ date, events = [], isDarkMode, onEventClick, onTimeSlotClick, weatherData = {} }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* This container will allow horizontal scrolling on mobile */}
      <div className="flex-1 overflow-auto">
        <div className="min-w-[800px] md:min-w-full">
          {/* Enhanced Week Header with responsive design */}
          <div className={`border-b sticky top-0 z-10 ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
            <div className="grid grid-cols-8 divide-x divide-gray-200 dark:divide-gray-700">
              {/* Time column header with gradient background */}
              <div className={`p-2 md:p-4 sticky left-0 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
          ...
          <div className="relative">
            ...
            {hours.map(hour => (
              ...
              >
                ...
                <div className={`p-2 text-right pr-2 md:pr-4 text-xs md:text-sm font-medium sticky left-0 ${
                  isDarkMode 
                  ...
                  {formatHour(hour)}
                  {/* Current time label */}
                  {new Date().getHours() === hour && weekDays.some(d => d.toDateString() === today.toDateString()) && (
                    <div 
                      className="absolute right-0 bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-md shadow-lg font-medium whitespace-nowrap z-20"
                      style={{ top: `calc(${(new Date().getMinutes() / 60) * 100}% - 8px)` }}
                    >
